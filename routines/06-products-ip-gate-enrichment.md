...
STEP 2½ — SOURCE-TRUTH RECONCILIATION (before writing a word of copy): compare the supplier/source spec against what the product PHOTOS actually show. A photo shows *appearance*, never *construction* — never assert a mechanism, material, or function the spec doesn't state (a demo image is not a feature; an AI room mockup is never construction evidence). Spec-vs-photo conflict on something material to what the buyer receives → the spec wins IF it's authoritative; if neither side can be verified, the verdict is UNCERTAIN/hold — never "upgrade" one side to match the other. Field failure this rule exists for: a painted demo read as a sealed liquid lamp; a plastic shade sold as glass.

STEP 3 — ENRICHMENT (PASS/CHANNEL-EXCLUDE only): save the original description for rollback → lean product description (60–120 words, brand voice, mobile-first — depth lives in structured fields, not prose; no health claims, no prices/shipping, no invented specs) → structured fields/metafields (specs, key features, common questions in Q&A form — the strongest AEO asset, care) → product type + tags + collections per your store's taxonomy → SEO title ≤60 (keyword-first + style keyword) + meta description 150–160 → realistic varied stock numbers → image ALT TEXT written from actually viewing each image (≥ the first 3 positions; literal, descriptive, <125 chars) and VERIFIED present via a read-back query BEFORE activation — never caption an image blind, and never activate with empty alt on the leading images. One product per write call; on errors remove the offending field, retry once, note the omission.

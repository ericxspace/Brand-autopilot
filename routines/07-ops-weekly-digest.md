...
- STRATEGIC GOAL: one consolidated, UNDER-ONE-PAGE view of the whole routine system so the founder never has to chase individual reports.
- SCOPE: read + summarize. NOT in scope: re-running any group's analytics, fixing anything, writing to any ledger or the store.
- AUTONOMY: fully autonomous, read-only (only write: the digest page).
...
DIGEST FORMAT (hard cap ~40 lines):
1. **Header** — store topline WoW.
2. **Ran / didn't run** — one line per routine: ✅ ran (n×) / ⚠ partial / ❌ no evidence + reason if visible.
2b. **Learning-loop health** — latest dated section in each learning file + rows appended to the outcome ledgers this week + the active experiment and its status. Flag any loop stale ≥2 weeks — a dead learning loop means the system has stopped improving.

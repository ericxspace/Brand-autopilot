...
- MIX ({{PIN_MODE}}, default `product-first`): {{PRODUCT_PINS_PER_DAY}} fresh Product Pins + {{INSPIRATION_PINS_PER_DAY}} fresh Inspiration Pins + 0 repins (default 6+4; keep the total ≤10/day — fresh pins drive 90%+ of Pinterest traffic; repeated content is the #1 distribution-collapse risk). State the mix in every run summary.
- `inspiration-first` MODE (recovery/growth variant — switch to it when linked product pins are being selectively suppressed, e.g. after a merchant strike, while linkless scene pins still distribute): invert the mix to a MAJORITY of linkless inspiration-tier pins built from multi-product room scenes (each scene styled with 2–4 real catalog products) + a SMALL product-pin control arm (~2/day, linked, back-catalog) to measure whether suppression is lifting. Each inspiration pin ships with a TAG MANIFEST (per pin: the scene image + the exact product identifiers in it) so the founder can manually product-tag the live pins on a daily cadence — tagged scene pins recover shopping intent without carrying the suppressed link. Source scenes from a per-week curated folder the founder/image-producer refreshes weekly (scenes are pre-scored and only curated images post); weekly audit tracks tagged-vs-untagged and the control arm as the recovery gate.
...
- Each run: randomly pick {{PRODUCT_PINS_PER_DAY}} DIFFERENT eligible folders (not excluded, not exhausted; vary product types — re-shuffle if fewer than ~2/3 are distinct types). From each, ONE unused image. `product-image-history.json` in {{OPS_FOLDER}} is the master ledger: any image ever pinned is permanently excluded. Pool short → fewer product pins, more inspiration pins + FLAG "pool low".
...
- All {{PINS_PER_DAY}} titles + first sentences unique tonight. Board from your real board list only; ≤2 pins/board/day across ≥6 boards.

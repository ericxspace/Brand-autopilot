...
- Pinterest volume & mix: `{{PINS_PER_DAY}}` total (default 10 — also the anti-spam ceiling; recommend trimming the TOTAL for new/recovering accounts) split into `{{PRODUCT_PINS_PER_DAY}}` / `{{INSPIRATION_PINS_PER_DAY}}` (default 6/4; inspiration always keeps the peak slots — it's what the algorithm distributes).
